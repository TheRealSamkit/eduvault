...
class AnnotationLayerBuilder {
	...
	async injectLinkAnnotations({
		...
		await this.annotationLayer.addLinkAnnotations(
			newLinks,
			this.linkService
		);
...
class BasePDFPageView {
	...
	constructor(options) {
		this.#enableHWA =
			#enableHWA in options
				? options.#enableHWA
				: options.enableHWA || false;
		this.eventBus = options.eventBus;
		this.id = options.id;
		this.pageColors = options.pageColors || null;
		this.renderingQueue = options.renderingQueue;
		this.#minDurationToUpdateCanvas =
...
class PDFPageDetailView extends BasePDFPageView {
	...
	#shouldRenderDifferentArea(visibleArea) {
		...
		if (
			...
			(maxDetailX < maxWidth &&
				paddingLeftSize / paddingRightSize > ratio) ||
			(minDetailY > 0 && paddingBottomSize / paddingTopSize > ratio) ||
			(maxDetailY < maxHeight &&
				paddingTopSize / paddingBottomSize > ratio)
		) {
			return true;
		}
		return false;
	...
	update({ visibleArea = null, underlyingViewUpdated = false } = {}) {
		...
		const { viewport, maxCanvasPixels, capCanvasAreaFactor } =
			this.pageView;
		...
		const maxY = Math.min(
			viewport.height,
			visibleArea.maxY + overflowHeight
		);
...
class PDFPageView extends BasePDFPageView {
	...
	constructor(options) {
		...
		this.maxCanvasDim =
			options.maxCanvasDim || AppOptions.get("maxCanvasDim");
		this.capCanvasAreaFactor =
			options.capCanvasAreaFactor ??
			AppOptions.get("capCanvasAreaFactor");
...
class PDFViewer {
	...
	forceRendering(currentlyVisiblePages) {
		...
		const ignoreDetailViews =
			this.#scaleTimeoutId !== null ||
			(this.#scrollTimeoutId !== null &&
				visiblePages.views.some(
					(page) => page.detailView?.renderingCancelled
				));
...
class SignatureManager {
	...
	constructor(
		...
		SignatureManager.#l10nDescription ||= Object.freeze({
			signature:
				"pdfjs-editor-add-signature-description-default-when-drawing",
		});
		dialog.addEventListener("close", this.#close.bind(this));
		...
		this.#description.addEventListener(
			"input",
			() => {
				this.#clearDescription.disabled =
	...
	#initTab(name) {
		...
		if (reset) {
			...
			this.#description.value = this.#tabsToAltText.get(
				this.#currentTab
			).value;
		}
		...
		this.#typeInput.addEventListener(
			...
			() => {
				...
				if (!this.#hasDescriptionChanged) {
					this.#tabsToAltText.get("type").default =
	...
	#initDrawTab(reset) {
		...
		const drawCallback = (e) => {
			...
			if (!this.#drawCurves) {
				...
				this.#drawPlaceholder.removeEventListener(
					"pointerdown",
					drawCallback
				);
				if (this.#description.value === "") {
					this.#l10n
						...
						.then((description) => {
							this.#tabsToAltText.get("draw").default =
								description;
				...
				.then(async (signatures) => [
					...
					await Promise.all(
						Array.from(signatures.values(), ({ signatureData }) =>
							SignatureExtractor.decompressSignature(
								signatureData
							)
						)
					),
	...
	async #add() {
		...
		if (this.#saveCheckbox.checked) {
			...
			if (uuid) {
				...
				console.warn(
					"SignatureManager.add: cannot save the signature."
				);
			}
		}
...
class EditDescriptionDialog {
	...
	async open(editor) {
		...
		this.#currentEditor = editor;
		this.#previousDescription = this.#description.value =
			editor.description;
...
const PDFViewerApplication = {
	...
	async _initializeViewerComponents() {
		...
		const renderingQueue = (this.pdfRenderingQueue =
			new PDFRenderingQueue());
			...
			appConfig.addSignatureDialog
				? new SignatureManager(
						...
						appConfig.annotationEditorParams
							?.editorSignatureAddSignature || null,
						overlayManager,
						l10n,
		...
		const pdfViewer = (this.pdfViewer = new PDFViewer({
			...
			minDurationToUpdateCanvas: AppOptions.get(
				"minDurationToUpdateCanvas"
			),
		}));
...
function getViewerConfiguration() {
	return {
		...
		toolbar: {
			...
			editorSignatureButton: document.getElementById(
				"editorSignatureButton"
			),
		...
		addSignatureDialog: {
			...
			drawPlaceholder: document.getElementById(
			...
			imagePlaceholder: document.getElementById(
			...
			errorCloseButton: document.getElementById(
